...
ckpt = None
configs = ['./configs/latent-diffusion/xcat-ldm-vq16-64ch.yaml']
opt.ldm = configs
print(opt.resume)
...
def main():
    ...
    all_names = sorted(glob.glob(datapath + '*_pair.npz'))
    val_split = max(20, int(len(all_names) * 0.1))
    # infer.py 在验证集上测试
    testname = all_names[:val_split]
