"""
验证集 block 4 的 3 个 slice 各 9 相位可视化
极简内存版本
"""
import os, gc, sys
import numpy as np
...
OUT_DIR    = os.path.join(DATA_ROOT, 'logs/raw_test_visualization/val_set_3slices_9phases')
...
# ---- Step 1: Draw per-slice grid ----
print("Drawing per-slice grids...")
for s in SLICES:
    gc.collect()
    fi = 684 + s
    ...
    fig.suptitle(
        f'Validation Set | Block 4 | Slice {s} | Fixed: fixed/{fi:03d}.npy\n'
    ...
    out = f'{OUT_DIR}/val_block4_slice{s:02d}_phases.png'
...
fig.suptitle('Validation Set: NCC(Fixed, Moving) vs Phase\nBlock 4 | Slices 5, 10, 15', fontsize=11, fontweight='bold')
plt.tight_layout(rect=[0, 0, 1, 0.93])
plt.savefig(f'{OUT_DIR}/val_block4_ncc_curves.png', dpi=120, bbox_inches='tight')
plt.close(fig); gc.collect()
print(f"  Saved: {OUT_DIR}/val_block4_ncc_curves.png")
...
plt.savefig(f'{OUT_DIR}/val_block4_ncc_heatmap.png', dpi=120, bbox_inches='tight')
plt.close(fig); gc.collect()
print(f"  Saved: {OUT_DIR}/val_block4_ncc_heatmap.png")
...
for s in SLICES:
    ...
    print(f'Slice {s:2d} (fixed/{fi:03d}): '
