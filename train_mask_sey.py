...
# NPZ 模式不再用 XCATNPZRegistration，改为按 split_indices.json 直接切片 + Dataset_epoch_with_name
...
parser.add_argument("--no_fg_mask", action="store_true",
                    dest="no_fg_mask",
                    help="关闭前景(人体)mask：图像域 loss 在全图(含黑背景)上计算，"
                         "step-10 mask 可视化也会跳过。适用于 SEY 这类前景已贴边的数据集。")
...
def masked_mse(pred, target, mask=None):
    ...
    mask=None 时退化为全图 MSE（F.mse_loss），用于 --no_fg_mask 模式。

    Args:
        pred, target: [B,1,H,W]
        mask: [B,1,H,W]，前景=1 背景=0；None 时全图 MSE
    """
    if mask is None:
...
def train():
    ...
    fg_status = 'OFF（--no_fg_mask，全图计算）' if opt.no_fg_mask else f'已开启 fg_thr={opt.fg_thr}'
    print(f"[INFO] 图像域 loss = {opt.loss_type.upper()}（前景遮罩{fg_status}）")
    ...
    if opt.xcat:
        ...
        from ldm.data.sey_registration import SEYRegistration as SEYOriginalRegistration
        train_loader = Data.DataLoader(
            SEYOriginalRegistration(data_root=opt.sey_path, split='train', normalize=False),
            batch_size=bs, shuffle=True, num_workers=0
        )
        val_loader = Data.DataLoader(
            SEYOriginalRegistration(data_root=opt.sey_path, split='val', normalize=False),
            batch_size=bs, shuffle=False, num_workers=0
        )
        test_loader = Data.DataLoader(
            SEYOriginalRegistration(data_root=opt.sey_path, split='test', normalize=False),
            batch_size=bs, shuffle=False, num_workers=0
        )
        print(f"SEY mode: train={len(train_loader.dataset)}, val={len(val_loader.dataset)}, test={len(test_loader.dataset)}")
        ...
        # NPZ 模式（新）：直接从 xcat_data 读原始 fixed/moving，与 fixed_motion 同源同预处理，
        # 唯一区别是 train 也只用 original moving（不展开 8 帧序列）。
        # 这样 NPZ 与 fixed_motion 成为"只差序列配对"的干净消融，NCC_Before 基线完全一致。
        # 数据根目录用 datapath（请把 --datapath 指向 xcat_data，而不是 *_pair.npz 目录）。
        npz_root = datapath
        train_loader = Data.DataLoader(
            XCATOriginalRegistration(data_root=npz_root, split='train', flip_p=0.5),
            batch_size=bs, shuffle=True, num_workers=0
        )
        val_loader = Data.DataLoader(
            XCATOriginalRegistration(data_root=npz_root, split='val', flip_p=0.0),
            batch_size=bs, shuffle=False, num_workers=0
        )
        test_loader = Data.DataLoader(
            XCATOriginalRegistration(data_root=npz_root, split='test', flip_p=0.0),
            batch_size=bs, shuffle=False, num_workers=0
        )
        print(f"NPZ mode (xcat_data original-only, same as fixed_motion minus seq): "
    ...
    if opt.xcat:
        ...
        # SEY-only 基线：无域适应，日志路径根据超参动态生成
        model_dir = f'./logs/SEY_Only_smooth{smooth}_beta{beta}_bending{opt.bending_w}_jacdet{opt.jacdet_w}/'
        csv_name  = model_dir.rstrip('/') + '.csv'
    else:
        model_dir = './logs/TransScorelm_Smooth_0.8_beta_0.8_7_15_15_625/'
        csv_name  = './logs/TransScorelm_Smooth_0.8_beta_0.8_7_15_15_625.csv'
    ...
    while step <= iteration:
        for X, Y, segx, segy, _ in train_loader:
            ...
            # 前景(人体)mask：用 fixed(Y) 估计，遮住黑色背景；--no_fg_mask 时跳过（fg=None）
            fg = None if opt.no_fg_mask else body_mask(Y, auto_percentile=True)

            # 第 10 步单独可视化一次前景 mask，确认背景是否被正确遮掉（--no_fg_mask 时跳过）
            if step == 10 and not opt.no_fg_mask:
                with torch.no_grad():
                    import matplotlib.pyplot as plt
            ...
            if step == 1 or step % 50 == 0:
                fg_str = f'fg_ratio: {fg.mean().item():.3f}' if fg is not None else 'fg_ratio: OFF (full image)'
            ...
            D_f_xy,score_out = model(X, Y, score0, score1, score2, score3)
            ...
            # 注：ncc_loss(a, b) 数学上对称 (cross/√(va vb))，
            # 所以下面两种写法数值上完全等价：
            #   ncc_loss(Y,    X_Y)   ← 语义对齐 "fixed, moving"
            #   ncc_loss(X_Y,  Y)     ← 这里实际传法（变量名跟函数槽反了，但不影响数值）
            if opt.loss_type == 'ncc':
                loss_image = ncc_loss(Y, X_Y, mask=fg)
            else:
                loss_image = masked_mse(Y, X_Y, fg)
            ...
            if step % 1000 == 0:
                with torch.no_grad():
                    ...
                    fg_cpu = fg[0, 0].cpu().numpy() if fg is not None else None

                    # 计算雅可比行列式
                    # 用与可视化脚本一致的 ×size/2 + jacobian_determinant_vxm 口径
            ...
            if (step % n_checkpoint == 0):
                with torch.no_grad():
                    ...
                    for xv, yv, xv_seg, yv_seg, _ in val_loader:
                        ...
                        # 前景 mask（用 fixed yv 估计）；--no_fg_mask 时跳过（fg_v=None）
                        fg_v = None if opt.no_fg_mask else body_mask(yv, auto_percentile=True)
                        ...
                        Dv_f_xy,score_out = model(xv, yv, vscore0, vscore1, vscore2, vscore3)
                        ...
                        for bs_index in range(bs):
                            ...
                            fgv_i = fg_v[bs_index,...].unsqueeze(0) if fg_v is not None else None
    ...
    if test_loader is not None:
        ...
        with torch.no_grad():
            for xt, yt, _, _, _ in test_loader:
                ...
                # 前景 mask（用 fixed yt 估计）；--no_fg_mask 时跳过（fg_t=None）
                fg_t = None if opt.no_fg_mask else body_mask(yt, auto_percentile=True)
                ...
                Dt_f_xy, score_output = model(xt, yt, tscore0, tscore1, tscore2, tscore3)
                ...
                for bs_index in range(bs):
                    ...
                    fgt_i = fg_t[bs_index, ...].unsqueeze(0) if fg_t is not None else None
