"""
Block 3 所有 slice 的 9 相位可视化
每个 slice 一张图: 9 rows x 3 cols (Fixed | Moving | Diff)
block 3 = indices 513..683
"""
import os, gc
import numpy as np
...
OUT_DIR    = os.path.join(DATA_ROOT, 'logs/raw_test_visualization/train_block3_per_slice_9phases')
...
for s in range(N_SLICES):
    ...
    fig.suptitle(
        f'Training Set | Block 3 | Slice {s} | Fixed: /fixed/{fi:03d}\n'
        f'9 Phases: moving/{BLOCK_START+s:03d} (P1) .. moving/{BLOCK_START+s+8*19:03d} (P9)',
        fontsize=11, fontweight='bold', y=0.995
    )
    plt.tight_layout(rect=[0, 0, 1, 0.98])
    out = f'{OUT_DIR}/train_block3_slice{s:02d}_9phases.png'
